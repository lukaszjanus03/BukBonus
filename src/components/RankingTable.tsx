...
import { BOOKMAKERS, Bookmaker } from '@/data/bookmakers';

interface RankingProps {
  currentDate?: string;
  customList?: Bookmaker[]; 
}

export default function Ranking({ currentDate, customList }: RankingProps) {
  const [expandedBookie, setExpandedBookie] = useState<number | null>(null);
  const displayDate = currentDate || '2026';
  const dataToShow = customList || BOOKMAKERS;
          ...
          {dataToShow.map((bookie, index) => (
            <div key={bookie.id} className="bg-white rounded-xl shadow-md border border-slate-100 overflow-hidden hover:shadow-lg transition-shadow duration-300 relative group">
              ...
              <div className="p-6 flex flex-col md:flex-row items-center gap-6">
                ...
                {/* Logo & Rating */}
                <div className="flex flex-col items-center justify-center w-full md:w-36 shrink-0">
                    {/* --- NAPRAWIONA SEKCJA LOGO --- */}
                    <div className="w-28 h-12 relative mb-2 flex items-center justify-center">
                        <Image 
                            src={bookie.logo} 
                            alt={`Kod promocyjny ${bookie.name}`}
                            fill // Używamy 'fill' zamiast width/height
                            sizes="(max-width: 768px) 100vw, 112px" // Podpowiedź dla przeglądarki
                            className="object-contain p-0.5" // object-contain zapewnia, że logo się nie rozciągnie
                        />
                    </div>
                    {/* -------------------------------- */}
                ...
                <div className="flex-grow text-center md:text-left border-t md:border-t-0 md:border-l border-slate-100 pt-4 md:pt-0 md:pl-6 w-full">
                    <Link href={`/poradnik/${bookie.slug}`} className="text-xl font-black text-slate-800 mb-2 hover:text-blue-600 transition-colors flex items-center justify-center md:justify-start gap-2">
                        {bookie.name}
                    ...
                    <div className="flex flex-wrap justify-center md:justify-start gap-2 mb-4">
                        {bookie.features.slice(0, 3).map((feature, i) => (
                            <span key={i} className="bg-slate-50 text-slate-600 text-[10px] font-bold px-2 py-1 rounded border border-slate-200">
                                {feature}
                            </span>
                        ))}
                    </div>
                    ...
                    <div className="flex flex-col md:flex-row items-center gap-3">
                        ...
                        <span className={`text-2xl font-black ${index === 0 ? 'text-red-600' : 'text-slate-900'}`}>
                            {bookie.bonus}
                        </span>
                    </div>
                </div>
                ...
                {/* CTA Buttons */}
                <div className="w-full md:w-48 shrink-0 flex items-center justify-center">
                    ...
                    >
                        <div className="py-3 px-1">
                            <span className="block font-black text-base uppercase tracking-wide leading-none">
                                Odbierz Bonus
                            </span>
                        </div>
                        
                        <div className="bg-black/20 rounded mx-0.5 py-2 flex items-center justify-center gap-1.5 border border-white/10 group-hover:bg-black/30 transition-colors mb-0.5">
              ...
              {/* Expanded Details */}
              <div className={`bg-slate-50 border-t border-slate-100 transition-all duration-300 ease-in-out overflow-hidden ${expandedBookie === bookie.id ? 'max-h-96 opacity-100 p-6' : 'max-h-0 opacity-0 p-0'}`}>
                  <div className="grid md:grid-cols-2 gap-6">
                      ...
                      <div className="flex items-center justify-center border-l border-slate-200 pl-6">
                           <div className="text-center flex flex-col gap-3">
                               ...
                               <a href={bookie.link} className="text-blue-600 font-bold text-sm hover:underline flex items-center justify-center gap-1">
                                   Przejdź do strony <i className="fas fa-external-link-alt text-xs"></i>
                               </a>
                               <Link href={`/poradnik/${bookie.slug}`} className="text-yellow-500 font-bold text-sm hover:text-yellow-600 flex items-center justify-center gap-1">
